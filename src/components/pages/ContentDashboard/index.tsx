...
export default function ContentDashboard () {
  ...
  const fetcher = async (apiCall: (router:any, params:string)=>Promise<any>, params:string) => {
    try {
  ...
  const { data: GaugeData, isLoading:isGaugeDataLoading } =
    useSWR(["gauge", params, getRealisasiAnggaran], ([, p, api]) => fetcher(api, p))
  const { data: PieData, isLoading:isPieDataLoading } =
    useSWR(["pie", params, getRealisasiBiayaPie], ([, p, api]) => fetcher(api, p))
  const { data: DonutData, isLoading:isDonutDataLoading } =
    useSWR(["donut", params, getRealisasiPendapatanDonut], ([, p, api]) => fetcher(api, p))
  const { data: BarData, isLoading:isBarDataLoading } =
    useSWR(["bar", params, getRealisasiBiayaChart], ([, p, api]) => fetcher(api, p))
  const { data: TargetAnggaran, isLoading:isTargetAnggaranLoading } =
    useSWR(["target", params, getTargetAnggaranDashboard], ([, p, api]) => fetcher(api, p))
  const { data: JumlahLPU, isLoading:isJumlahLPULoading } =
    useSWR(["jumlahlpu", params, getJumlahLPU], ([, p, api]) => fetcher(api, p))
  const { data: JumlahMitraLPU, isLoading:isJumlahMitraLPULoading } =
    useSWR(["mitralpu", params, getJumlahMitraLPU], ([, p, api]) => fetcher(api, p))

  const isLoadingAll = !GaugeData || !PieData || !DonutData || !BarData || !TargetAnggaran || !JumlahLPU || !JumlahMitraLPU

  const handleApplyFilters = (newParams: string, raw: DashboardFilterValues) => {
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-4">
              ...
              <Card className="relative">
                ...
                <CardHeader><CardTitle>Jumlah Mitra LPU</CardTitle></CardHeader>
                <CardContent className="pl-2 w-min mx-auto my-auto items-center">
                  <span className="text-4xl font-bold">
                    {typeof JumlahMitraLPU?.count === "number" ? numFormatter(JumlahMitraLPU.count) : "-"}
                  </span>
                </CardContent>
              </Card>

              <Card><CardHeader><CardTitle>Jumlah Pengguna Layanan Kantor LPU</CardTitle></CardHeader>
                <CardContent className="pl-2 w-min mx-auto my-auto items-center">
                  <span className="text-4xl font-bold">-</span>
                </CardContent>
              </Card>

              <Card><CardHeader><CardTitle>Jumlah Pengguna Layanan Mitra LPU</CardTitle></CardHeader>
                <CardContent className="pl-2 w-min mx-auto my-auto items-center">
                  <span className="text-4xl font-bold">-</span>
                </CardContent>
              </Card>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              <Card className="relative">
                ...
                <CardHeader><CardTitle>Realisasi Anggaran</CardTitle></CardHeader>
                <CardContent className="pl-2 w-min mx-auto my-auto items-center">
              ...
              <Card className="relative">
                ...
                <CardHeader><CardTitle>Realisasi Biaya</CardTitle></CardHeader>
              ...
              <Card className="relative">
                ...
                <CardHeader><CardTitle>Realisasi Pendapatan</CardTitle></CardHeader>
