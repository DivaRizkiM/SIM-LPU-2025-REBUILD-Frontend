...
import { getExportMonitoringKantorUsulan, getKabKota, getKecamatan, getKelurahan, getMonitoringKantorUsulan, getProvinsi } from "../../../../../services";
import { useRouter } from "next/navigation";
import { KabKotaI, KecamatanI, KelurahanI, MonitoringKantorUsulanI, ProvinsiI } from "../../../../../services/types";
...
const MonitoringKantorUsulan:NextPage = ()=>{
    ...
    const [dataSource, setDataSource] = useState<MonitoringKantorUsulanI[]>()
    ...
    const fetchData = async(pagination=1, id_provinsi='',id_kabupaten_kota='',id_kecamatan='',id_kelurahan='',tahun='',bulan='') => {
        ...
        if (offset < 0)return
        await getMonitoringKantorUsulan(router, params)
        .then((res)=> {
            const { 
                offset,
                limit,
    ...
    const exportData = async(dataForm: z.infer<typeof FormSchema>)=>{
        ...
        const params = buildQueryParam(tempParams) || '';
        await getExportMonitoringKantorUsulan(router,params)
            .then((res)=>{
                const url = window.URL.createObjectURL((res.data as any));
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                const fileNameParts = [`monitoring_kantor_usulan`];
                        ...
                        { dataSource && dataSource.length > 0 ? (dataSource?.map((data,key)=>(
                            <tr key={key}>
                               <td>{offset + key + 1}</td>
                                ...
                                <td>{data.aspek_pemahaman_pegawai}</td>
                                <td>{data.aspek_pemahaman_wilayah}</td>
                                <td>{data.aspek_sarana_prasarana}</td>
