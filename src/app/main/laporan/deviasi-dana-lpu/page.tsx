...
import { getDeviasiDanaLPU, getExportDeviasiDanaLPU, getKPRKByRegional, getRegional } from "../../../../../services";
import { useRouter } from "next/navigation";
import { RegionalI } from "../../referensi/regional/columns";
import { DeviasiDanaI } from "../../../../../services/types";
...
const RealisasiDanaLPU:NextPage = ()=>{
    ...
    const [dataSource, setDataSource] = useState<DeviasiDanaI[]>()
    ...
    const fetchData = async(pagination=1, tahun='',triwulan='',regional='',kcu='') => {
        ...
        if (offset < 0)return
        await getDeviasiDanaLPU(router, params)
        .then((res)=> {
            const { 
                offset,
                limit,
    ...
    const exportData = async()=>{
        ...
        const params = buildQueryParam(tempParams) || '';
        await getExportDeviasiDanaLPU(router,params)
            .then((res)=>{
                const url = window.URL.createObjectURL((res.data as any));
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                const fileNameParts = [`deviasi_dana_lpu`];
    ...
    return(
        <div className="px-3 lg:px-7 mt-5">
            ...
            <h1 className="text-center my-7 md:my-3 font-bold">DEVIASI DANA LPU</h1>
                ...
                <table className={style.table}>
                    <thead className="[&:nth-child(3):bg-red-900]">
                        ...
                        <tr>
                            ...
                            <th>Realisasi Pendapatan - Biaya</th>
                            <th>Deviasi</th>
                        </tr>
                    </thead>
                    <tbody>
                        { dataSource && dataSource.length > 0 ? (dataSource?.map((data,key)=>(
                            <tr key={key}>
                                ...
                                <td>{data.nama_kpc}</td>
                                <td>{data.sum_alokasi_dana_lpu}</td>
                                <td>{numFormatter(data.realisasi)}</td>
                                <td>{numFormatter(data.deviasi)}</td>
                            </tr>
                        ))) : (
...
export default RealisasiDanaLPU