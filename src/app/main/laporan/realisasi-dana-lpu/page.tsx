...
import { getExportRealisasiDanaLPU, getKPRKByRegional,  getRealisasiDanaLPU, getRegional } from "../../../../../services";
import { useRouter } from "next/navigation";
import { RegionalI } from "../../referensi/regional/columns";
import { ILaporanVerifikasiPendapatan, RealisasiDanaI } from "../../../../../services/types";
...
const DeviasiDanaLPU:NextPage = ()=>{
    ...
    const [dataSource, setDataSource] = useState<RealisasiDanaI[]>()
    ...
    const fetchData = async(pagination=1, tahun='',triwulan='',regional='',kcu='') => {
        ...
        if (offset < 0)return
        await getRealisasiDanaLPU(router, params)
        .then((res)=> {
            const { 
                offset,
                limit,
    ...
    const exportData = async()=>{
        ...
        const params = buildQueryParam(tempParams) || '';
        await getExportRealisasiDanaLPU(router,params)
            .then((res)=>{
                ...
                a.href = url;
                const fileNameParts = [`realisasi_dana_lpu`];
    ...
    return(
        <div className="px-3 lg:px-7 mt-5">
            ...
            <h1 className="text-center my-7 md:my-3 font-bold">REALISASI DANA LPU</h1>
            <div className={`relative overflow-x-scroll`}>
                <table className={style.table}>
                    <thead className="[&:nth-child(3):bg-red-900]">
                        ...
                        <tr>
                            <th rowSpan={3}>No</th>
                            <th rowSpan={3}>Nomor Dirian</th>
                            <th rowSpan={3}>Nama Kantor POS</th>
                            <th colSpan={3}>Pendapatan</th>
                            <th rowSpan={3}>Biaya</th>
                            <th rowSpan={3}>Selisih</th>
                        </tr>
                        {/* Second row of header group */}
                        <tr>
                            <th colSpan={2}>Transfer Pricing</th>
                            <th rowSpan={2}>Sisa Layanan</th>
                        </tr>
                        <tr>
                            <th>Incoming</th>
                            <th>Outgoing</th>
                    ...
                    <tbody>
                        { dataSource && dataSource.length > 0 ? (dataSource?.map((data,key)=>(
                            <tr key={key}>
                                ...
                                <td>{data.nama_kpc}</td>
                                <td>{numFormatter(data.verifikasi_incoming)}</td>
                                <td>{numFormatter(data.verifikasi_outgoing)}</td>
                                <td>{numFormatter(data.verifikasi_sisa_layanan)}</td>
                                <td>{numFormatter(data.biaya)}</td>
                                <td>{numFormatter(data.selisih)}</td>
                            </tr>
                        ))) : (
