...
import { getExportVerifikasiPendapatan, getKPRK, getKPRKByRegional, getKertasKerjaVerifikasi, getRegional, getVerifikasiPendapatan } from "../../../../../services";
...
const LaporanVerifikasiPendapatan:NextPage = ()=>{
    ...
    const fetchData = async(pagination=1, tahun='',triwulan='',regional='',kcu='') => {
        ...
        if (offset < 0)return
        await getVerifikasiPendapatan(router, params)
        .then((res)=> {
            const { 
                offset,
                limit,
    ...
    const exportData = async()=>{
        ...
        const params = buildQueryParam(tempParams) || '';
        await getExportVerifikasiPendapatan(router,params)
            .then((res)=>{
                const url = window.URL.createObjectURL((res.data as any));
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                const fileNameParts = [`laporan_verifikasi_pendapatan`];
    ...
    return(
        <div className="px-3 lg:px-7 mt-5">
            ...
            <h1 className="text-center my-7 md:my-3 font-bold">LAPORAN VERIFIKASI PENDAPATAN</h1>
