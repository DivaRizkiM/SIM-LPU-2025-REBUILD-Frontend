...
import { getExportPrognosaPendapatan, getExportVerifikasiBiaya, getExportVerifikasiPendapatan, getKPRK, getKPRKByRegional, getKertasKerjaVerifikasi, getRegional, getVerifikasiBiaya, getVerifikasiPendapatan } from "../../../../../services";
import { useRouter } from "next/navigation";
import { RegionalI } from "../../referensi/regional/columns";
import {  LaporanBiayaI } from "../../../../../services/types";
import { QueryParams, buildQueryParam, isLastPage, numFormatter } from "../../../../../helper";
...
const LaporanVerifikasiBiaya:NextPage = ()=>{
    ...
    const [dataSource, setDataSource] = useState<LaporanBiayaI[]>()
    ...
    const fetchData = async(pagination=1, tahun='',triwulan='',regional='',kcu='') => {
        ...
        if (offset < 0)return
        await getVerifikasiBiaya(router, params)
        .then((res)=> {
            const { 
                offset,
                limit,
    ...
    const exportData = async()=>{
        ...
        const params = buildQueryParam(tempParams) || '';
        await getExportVerifikasiBiaya(router,params)
            .then((res)=>{
                const url = window.URL.createObjectURL((res.data as any));
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                const fileNameParts = [`verifikasi_biaya`];
    ...
    const handleNextClick = () => {
        setPage(page+1)
        fetchData(page + 1, tahunSelected, triwulanSelected,regionalSelected);
    };
    ...
    return(
        <div className="px-3 lg:px-7 mt-5">
            ...
            <h1 className="text-center my-7 md:my-3 font-bold">LAPORAN VERIFIKASI BIAYA</h1>
            <div className={`relative overflow-x-scroll`}>
                <table className={style.table}>
                    <thead className="[&:nth-child(3):bg-red-900]">
                        ...
                        <tr>
                            <th rowSpan={2}>No</th>
                            <th rowSpan={2}>Nama Regional</th>
                            <th rowSpan={2}>Nama KC / KCU</th>
                            <th rowSpan={2}>Nomor Dirian</th>
                            <th rowSpan={2}>Nama Kantor POS</th>
                    ...
                    <tbody>
                        { dataSource && dataSource.length > 0 ? (dataSource?.map((data,key)=>(
                            <tr key={key}>
                                ...
                                <td>{data.nomor_dirian}</td>
                                <td>{data.nama_kpc}</td>
                                <td>{numFormatter(data.total_biaya_pegawai)}</td>
                                <td>{numFormatter(data.total_biaya_operasi)}</td>
                                <td>{numFormatter(data.total_biaya_pemeliharaan)}</td>
                                <td>{numFormatter(data.total_biaya_administrasi)}</td>
                                <td>{numFormatter(data.total_biaya_penyusutan)}</td>
                                <td>{numFormatter(data.jumlah_biaya)}</td>
                            </tr>
                        ))) : (
...
export default LaporanVerifikasiBiaya