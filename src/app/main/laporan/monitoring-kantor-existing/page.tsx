...
import { getExportMonitoringKantorExisting,  getKabKota, getKecamatan, getKelurahan, getMonitoringKantorExisting, getProvinsi, getRegional } from "../../../../../services";
import { useRouter } from "next/navigation";
import { KabKotaI, KecamatanI, KelurahanI, MonitoringKantorExistingI, ProvinsiI } from "../../../../../services/types";
...
const FormSchema = z.object({
    id_kecamatan: z.string({required_error: 'Kecamatan tidak boleh kosong'}).min(1, {
      message: "Kecamatan harus lebih dari 1 karakter.",
    }).optional(),
    id_provinsi: z.string({required_error: 'Provinsi tidak boleh kosong'}).min(1, {
      message: "Provinsi harus lebih dari 1 karakter.",
    }).optional(),
    id_kabupaten_kota: z.string({required_error: 'Kab/Kota tidak boleh kosong'}).min(1, {
      message: "Kab/Kota harus lebih dari 1 karakter.",
    }).optional(),
    id_kelurahan: z.string({required_error: 'Kelurahan tidak boleh kosong'}).min(1, {
      message: "Kelurahan harus lebih dari 1 karakter.",
    }).optional(),
    tahun: z.string({required_error: 'Tahun tidak boleh kosong'}).min(1, {
        message: "Tahun harus lebih dari 1 karakter.",
      }).optional(),
      bulan: z.string({required_error: 'Bulan tidak boleh kosong'}).min(1, {
        message: "Bulan harus lebih dari 1 karakter.",
      }).optional(),
  })

const MonitoringKantorExisting:NextPage = ()=>{
    const router = useRouter()
    const [page, setPage] = useState<number>(1);
    const pageSize:number = 10;
    const [offset,setOffset] = useState<number>(0)
    const [isLoading, setIsLoading] = useState<boolean>(false)
    const [dataSource, setDataSource] = useState<MonitoringKantorExistingI[]>()
    ...
    const fetchData = async(pagination=1, id_provinsi='',id_kabupaten_kota='',id_kecamatan='',id_kelurahan='',tahun='',bulan='') => {
        ...
        if (offset < 0)return
        await getMonitoringKantorExisting(router, params)
        .then((res)=> {
            const { 
                offset,
                limit,
    ...
    const exportData = async()=>{
        ...
        const params = buildQueryParam(tempParams) || '';
        await getExportMonitoringKantorExisting(router,params)
            .then((res)=>{
                const url = window.URL.createObjectURL((res.data as any));
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                const fileNameParts = [`monitoring_kantor_existing`];
        ...
        <div className="px-3 lg:px-7 mt-5">
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="grid lg:grid-cols-3 gap-3">
                    <Button className="" onClick={exportData} type="button" disabled>
                        {isExportLoading ? (
            ...
            <h1 className="text-center my-7 md:my-3 font-bold">Monitoring Kantor Existing</h1>
            <div className={`relative overflow-x-scroll`}>
                <table className={style.table}>
                    <thead className="[&:nth-child(3):bg-red-900]">
                        ...
                        <tr>
                            <th>Aspek Operasional (25%)</th>
                            <th>Aspek Sarana (25%)</th>
                            <th>Aspek Wilayah (25%)</th>
                            <th>Aspek Keuangan (25%)</th>
                        </tr>
                    </thead>
                    <tbody>
                        { dataSource && dataSource.length > 0 ? (dataSource?.map((data,key)=>(
                            <tr key={key}>
                                ...
                                <td>{data.aspek_operasional}</td>
                                <td>{data.aspek_sarana}</td>
                                <td>{data.aspek_wilayah}</td>
                                <td>{data.aspek_keuangan}</td>
...
export default MonitoringKantorExisting