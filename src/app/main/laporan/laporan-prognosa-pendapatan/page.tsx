...
import { getExportPrognosaPendapatan, getExportVerifikasiPendapatan, getKPRK, getKPRKByRegional, getKertasKerjaVerifikasi, getPrognosaPendapatan, getRegional, getVerifikasiPendapatan } from "../../../../../services";
import { useRouter } from "next/navigation";
import { RegionalI } from "../../referensi/regional/columns";
import { ILaporanVerifikasiPendapatan } from "../../../../../services/types";
...
const LaporanVerifikasiPendapatan:NextPage = ()=>{
    ...
    const [dataSource, setDataSource] = useState<ILaporanVerifikasiPendapatan[]>()
    ...
    const fetchData = async(pagination=1, tahun='',triwulan='',regional='',kcu='') => {
        ...
        if (offset < 0)return
        await getPrognosaPendapatan(router, params)
        .then((res)=> {
            const { 
                offset,
                limit,
    ...
    const exportData = async()=>{
        ...
        const params = buildQueryParam(tempParams) || '';
        await getExportPrognosaPendapatan(router,params)
            .then((res)=>{
    ...
    return(
        <div className="px-3 lg:px-7 mt-5">
            ...
            <h1 className="text-center my-7 md:my-3 font-bold">LAPORAN PROGNOSA PENDAPATAN</h1>
                    ...
                    <tbody>
                        { dataSource && dataSource.length > 0 ? (dataSource?.map((data,key)=>(
                            <tr key={key}>
                                ...
                                <td>{data.nama_kpc}</td>
                                <td>{numFormatter(data.total_lpu)}</td>
                                <td>{numFormatter(data.total_lpk)}</td>
                                <td>{numFormatter(data.total_lbf)}</td>
                                <td>{numFormatter(data.jumlah_pendapatan)}</td>
                            </tr>
                        ))) : (
...
export default LaporanVerifikasiPendapatan
