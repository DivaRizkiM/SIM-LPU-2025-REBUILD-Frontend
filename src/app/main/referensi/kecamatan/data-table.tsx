...
import { deleteKecamatan, syncKecamatan } from "../../../../../services"
...
export function DataTable<TData, TValue>({
  ...
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>(
    ...
    const showAlertDelete = (data:KecamatanI) => {      
      ctx.dispatch({
        alertDialog: {
          type: 'delete',
    ...
    const onDeleteData = async (data:KecamatanI) => {      
      await deleteKecamatan(router,data.id)
        .then(()=>{
          toast({
    ...
    const onClickEditData = (data:KecamatanI)=>{
      ctx.dispatch({
        isModal: {
          title: 'Mengubah Data Provinsi',
          type: "drawer",
    ...
    const onSyncClick = ()=> {
      ctx.dispatch({
        alertDialog: {
          type: 'sync',
          title: `Apakah anda yakin ingin menyinkronkan Kecamatan?`,
          onSubmit: async()=>{
            await syncKecamatan(router)
              .then((res)=>{
                toast({
