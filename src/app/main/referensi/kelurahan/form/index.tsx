...
import { getKabKota, getProvinsi, postKelurahan, putKelurahan } from "../../../../../../services"
...
const FormData:FC<FromDataI> = ({ trigger,data })=> {
  ...
  const onSubmit = async (dataForm: z.infer<typeof FormSchema>) =>{
    ...
    if (!data){
      ...
      .then((res)=>{
        toast({
          title: `Berhasil menambahkan data kelurahan ${form.getValues('nama')}!`,
        })
        trigger()
      ...
      .then(()=>{
        toast({
          title: `Berhasil mengubah data kelurahan ${form.getValues('nama')}!`,
        })
        trigger()
        return destroyDrawerModal(ctx)
      })
