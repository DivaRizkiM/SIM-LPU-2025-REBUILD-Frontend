...
import { KecamatanI } from "../../kecamatan/columns";
...
const FormSchema = z.object({
  ...
  nama: z
    .string({ required_error: "Nama Kecamatan tidak boleh kosong" })
    .min(1, {
      message: "Nama Kecamatan harus lebih dari 1 karakter.",
    }),
  id_provinsi: z
    .string({ required_error: "Nama Provinsi tidak boleh kosong" })
    .min(1, {
      message: "Nama Provinsi harus lebih dari 1 karakter.",
    }),
  id_kabupaten_kota: z
    .string({ required_error: "Nama Kab/Kota tidak boleh kosong" })
    .min(1, {
...
const FormData: FC<FromDataI> = ({ trigger, data }) => {
  ...
  const [allKecamatan, setAllKecamatan] = useState<Array<any>>([]);
  ...
  const onSubmit = async (dataForm: z.infer<typeof FormSchema>) => {
    setIsLoading(true);
    if (!data) {
      await postKelurahan(router, dataForm)
        .then((res) => {
          toast({
            title: `Berhasil menambahkan data kelurahan ${form.getValues(
              "nama"
            )}!`,
          });
          trigger();
      ...
      await putKelurahan(router, dataForm, data.id)
        .then(() => {
          toast({
            title: `Berhasil mengubah data kelurahan ${form.getValues(
              "nama"
            )}!`,
          });
          trigger();
  ...
  const firstInit = async () => {
    ...
    await Promise.all([
      ...
      getKecamatan(router, "?limit=9999"),
    ])
      .then((responses) => {
        const [province, kab_kota, kecamatan] = responses;
        ...
        setAllKecamatan(kecamatan.data.data);
        if (data) {
          getKotaByProvince(data.id_provinsi, kab_kota.data.data);
          getKecamatanByKota(data.id_kabupaten_kota, kecamatan.data.data);
  ...
  const getKotaByProvince = (
    id: string | number,
    DataAllKabKota: Array<any> = allKabKota
  ) => {
    setIsKotaLoading(true);
    try {
      const kotaYangCocok: KabKotaI[] = DataAllKabKota.filter(
        (item: KabKotaI) => item.id_provinsi.toString() === id.toString()
      );
  ...
  const getKecamatanByKota = (
    id: string | number,
    DataAllKecamatan: Array<any> = allKecamatan
  ) => {
    setIsKecamatanLoading(true);
    try {
      const kecamatanYangCocok: KecamatanI[] = DataAllKecamatan.filter(
        (item: KecamatanI) =>
          item.id_kabupaten_kota.toString() === id.toString()
      );
      const kecamatanFiltered = kecamatanYangCocok.map((item: KecamatanI) => ({
        value: item.id.toString(),
        label: item.nama,
      }));
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Combobox
                ...
                isLoading={isKecamatanLoading} // <- ganti ini
                disabled={kecamatanOptions.length === 0 || isKecamatanLoading} // <- ganti ini
              />
            </FormItem>
          )}
        />
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Nama Kelurahan</FormLabel>
              <FormControl>
                <Input placeholder="Kelurahan" {...field} autoFocus={false} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <div className="space-x-2">
          <Button
            type="button"
            variant={"outline"}
            onClick={() => destroyDrawerModal(ctx)}
          >
            Batal
          </Button>
          <Button type="submit">
