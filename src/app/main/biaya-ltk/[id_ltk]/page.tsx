...
const Detail: NextPage = () => {
    ...
    const [dataVerifications, setDataVerifications] = useState<Array<IFormLtkVerifikasi>>([])
    ...
    const firstInit = async () => {
        ...
        await getDetailLtk(router, payload)
            .then((res) => {
                ...
                const tempDataVerifs: IFormLtkVerifikasi[] = dataResponse.map((data) => {
                    return {
                        id_ltk: data.id.toString(),
                        verifikasi_akuntansi: data.verifikasi_akuntansi || "0,00",
                        isVerifikasiAkuntansiSesuai: cleanCurrencyFormat(data.verifikasi_akuntansi) === "0,00" ? "" : (cleanCurrencyFormat(data.verifikasi_akuntansi || "0,00") === cleanCurrencyFormat(data.mtd_akuntansi || "0,00") ? "1" : '0'),
                        isVerifikasiProporsiSesuai: cleanCurrencyFormat(data.verifikasi_proporsi) === "0,00" ? "" : (cleanCurrencyFormat(data.verifikasi_proporsi || "0,00") === cleanCurrencyFormat(data.proporsi_rumus || "0,00") ? "1" : '0'),
                        isVerifikasiPsoSesuai: cleanCurrencyFormat(data.verifikasi_pso) === "0,00" ? "" : (cleanCurrencyFormat(data.verifikasi_pso || "0") === cleanCurrencyFormat(data.biaya_pso || "0,00") ? "1" : '0'),
        ...
        switch (fieldName) {
            case "akuntansi":
                tempData[indexSelected].verifikasi_akuntansi =
                    val === "1" ? cleanCurrencyFormat(selectedData?.mtd_akuntansi || "0,00") : "0,00";
                tempData[indexSelected].isVerifikasiAkuntansiSesuai = val;
                break;

            case "pso":
                tempData[indexSelected].verifikasi_pso =
                    val === "1" ? cleanCurrencyFormat(selectedData?.biaya_pso || "0,00") : "0,00";
                tempData[indexSelected].isVerifikasiPsoSesuai = val;
                break;

            case "proporsi":
                tempData[indexSelected].verifikasi_proporsi =
                    val === "1" ? cleanCurrencyFormat(selectedData?.proporsi_rumus || "0,00") : "0,00";
    ...
    const onSubmitVerifikasi = async () => {
        ...
        const tempData: IFormLtkVerifikasi[] = dataVerifications.map((data) => {
            return {
                ...
                verifikasi_proporsi: data.verifikasi_proporsi || "0",
                catatan_pemeriksa: data.catatan_pemeriksa || ""
            }
        })
        const payload = {
    ...
    return (
        <div className="relative px-2 md:px-4 py-1 md:py-2 h-full overflow-hidden">
            ...
            <h3 className="scroll-m-20 text-xl font-semibold tracking-tight mb-3 border-b pb-2">VERIFIKASI LTK - DETAIL</h3>
                            ...
                            <Button
                                ...
                                variant={
                                    data?.id.toString() === selectedID
                                        ? 'default'
                                        : checkVerifSync(dataVerifications[key].verifikasi_akuntansi, cleanCurrencyFormat(data.verifikasi_akuntansi || '0'))
                                }
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                            ...
                            <div className="grid gap-2 md:grid-cols-4 items-center">
                                ...
                                <Input value={selectedData?.mtd_akuntansi || '0'} className="w-full col-span-3 bg-secondary" readOnly />
                            </div>
                            <div className="grid gap-2 md:grid-cols-4 items-center">
                                <Label>MTD Biaya Final</Label>
                                <Input value={(selectedData?.mtd_biaya_hasil || '0')} className="w-full col-span-3 bg-secondary" readOnly />
                            </div>
                            <div className="flex gap-4 items-center mt-4">
                                <Label className="font-semibold text-lg">Lampiran: </Label>
                                {(selectedData && selectedData?.url_file !== "" && selectedData.url_file !== "")
                                    ? (
                                        <a
                        ...
                        <div className="space-y-4">
                            <div className='space-y-2'>
                                <div className="grid gap-2 md:grid-cols-4 items-center">
                                    ...
                                    <Input value={selectedData?.mtd_akuntansi} className="w-full col-span-3 bg-secondary" readOnly />
                                </div>
                                <div className="grid gap-2 md:grid-cols-4 items-center">
                                    <div>
                                        <Label>Pilih Kondisi <span className="text-red-500">*</span></Label>
                                        {!dataVerifications[indexSelected]?.isVerifikasiAkuntansiSesuai && (
                                            <div className="text-xs text-red-600">Wajib diisi</div>
                                        )}
                                    </div>
                                    <div className="w-full col-span-3">
                                        <Select
                                            value={dataVerifications[indexSelected]?.isVerifikasiAkuntansiSesuai}
                                            onValueChange={(val: "0" | "1") => selectHandler(val, "akuntansi")}
                                        >
                                            <SelectTrigger
                                                className={!dataVerifications[indexSelected]?.isVerifikasiAkuntansiSesuai ? 'border-red-600' : ''}
                                            >
                                ...
                                <div className="grid gap-2 md:grid-cols-4 items-center">
                                    <div>
                                        <Label>Nominal Verifikasi <span className="text-red-500">*</span></Label>
                                        {dataVerifications[indexSelected]?.verifikasi_akuntansi === "" && (
                                            <div className="text-xs text-red-600">Wajib diisi</div>
                                        )}
                                    </div>
                                    <CurrencyInput
                                        value={dataVerifications[indexSelected]?.verifikasi_akuntansi
                                        }
                                        name="verifikasi_akuntansi"
                                        className={`
                                        w-full col-span-3 
                                        ${dataVerifications[indexSelected]?.isVerifikasiAkuntansiSesuai === "1" || dataVerifications[indexSelected]?.isVerifikasiAkuntansiSesuai === "" ? "bg-secondary" : ''}
                                        ${dataVerifications[indexSelected]?.verifikasi_akuntansi === "" ? 'border-red-600' : ''}
                                    `}
                                        readOnly={dataVerifications[indexSelected]?.isVerifikasiAkuntansiSesuai === "1" || dataVerifications[indexSelected]?.isVerifikasiAkuntansiSesuai === ""}
                            ...
                            <div className='space-y-2'>
                                <div className="grid gap-2 md:grid-cols-4 items-center">
                                    ...
                                    <Input value={selectedData?.biaya_pso} className="w-full col-span-3 bg-secondary" readOnly />
                                </div>
                                <div className="grid gap-2 md:grid-cols-4 items-center">
                                    <div>
                                        <Label>Nominal Verifikasi <span className="text-red-500">*</span></Label>
                            ...
                            <div className='space-y-2'>
                                <div className="grid gap-2 md:grid-cols-4 items-center">
                                    <Label>Proporsi Detail Rumus</Label>
                                    <Input value={selectedData?.proporsi_rumus} className="w-full col-span-3 bg-secondary" readOnly />
                                </div>
                                <div className="grid gap-2 md:grid-cols-4 items-center">
                                    <div>
                                        <Label>Pilih Kondisi <span className="text-red-500">*</span></Label>
                                        {!dataVerifications[indexSelected]?.isVerifikasiProporsiSesuai && (
                                            <div className="text-xs text-red-600">Wajib diisi</div>
                                        )}
                                    </div>
                                    <div className="w-full col-span-3">
                                        <Select
                                            value={dataVerifications[indexSelected]?.isVerifikasiProporsiSesuai}
                                            onValueChange={(val: "0" | "1") => selectHandler(val, "proporsi")}
                                        >
                                            <SelectTrigger
                                                className={!dataVerifications[indexSelected]?.isVerifikasiProporsiSesuai ? 'border-red-600' : ''}
                                            >
                                                <SelectValue placeholder="Pilih Kondisi" />
                                            </SelectTrigger>
                                            <SelectContent>
                                ...
                                <div className="grid gap-2 md:grid-cols-4 items-center">
                                    <div>
                                        <Label>Nominal Verifikasi <span className="text-red-500">*</span></Label>
                                        {dataVerifications[indexSelected]?.verifikasi_proporsi === "" && (
                                            <div className="text-xs text-red-600">Wajib diisi</div>
                                        )}
                                    </div>
                                    <CurrencyInput
                                        value={dataVerifications[indexSelected]?.verifikasi_proporsi
                                        }
                                        name="verifikasi_proporsi"
                                        className={`
                                        w-full col-span-3 
                                        ${dataVerifications[indexSelected]?.isVerifikasiProporsiSesuai === "1" || dataVerifications[indexSelected]?.isVerifikasiProporsiSesuai === "" ? "bg-secondary" : ''}
                                        ${dataVerifications[indexSelected]?.verifikasi_proporsi === "" ? 'border-red-600' : ''}
                                    `}
                                        readOnly={dataVerifications[indexSelected]?.isVerifikasiProporsiSesuai === "1" || dataVerifications[indexSelected]?.isVerifikasiProporsiSesuai === ""}
                                        onChange={handleInput}
                                        symbol='%'
                                    />
                                </div>
