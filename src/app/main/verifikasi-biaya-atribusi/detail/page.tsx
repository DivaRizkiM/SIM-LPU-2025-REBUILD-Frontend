...
import { getDetailBiayaAtribusi, postVerifikasiBiayaAtribusi } from "../../../../../services";
...
const Detail: NextPage = () => {
    ...
    const [dataVerifications, setDataVerifications] = useState<Array<IFormVerifikasiBiayaAtribusi>>([])
        ...
        await getDetailBiayaAtribusi(router, params)
            .then((res) => {
                ...
                if (((res.data as any).isLock)) {
                    return router.push(`./pelaporan-verifikasi?ba_id=${ba_id}&kcu_id=${id_kcu}&triwulan=${getQuarter(parseInt(bulan || ''))}&tahun=${tahun}`)
                }
                ...
                const tempDataVerifs: IFormVerifikasiBiayaAtribusi[] = dataResponse.map((data) => {
                    return {
    ...
    const onSubmitVerifikasi = async () => {
        ...
        const tempData: IFormVerifikasiBiayaAtribusi[] = dataVerifications.map((data) => {
            return {
    ...
    return (
        <div className="relative px-2 md:px-4 py-1 md:py-2 h-full overflow-hidden">
            <h3 className="scroll-m-20 text-xl font-semibold tracking-tight mb-3 border-b pb-2">VERIFIKASI BIAYA ATRIBUSI - DETAIL</h3>
                ...
                <div className={cn(animateTrigger ? 'translate-x-5 md:translate-x-10 opacity-50' : 'translate-x-0 opacity-1', 'rounded-md border md:px-3 p-2 w-full h-full overflow-y-scroll overflow-x-hidden transition-all duration-200')} ref={containerRef}>
                    ...
                    <div className="grid lg:grid-cols-2 gap-4 mb-32">
                        ...
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                            ...
                            <div className="w-full md:col-span-3 opacity-80 border-dotted decoration-muted-foreground border-l ps-2 md:ps-3 flex">{selectedData?.nama_kcu}</div>
                        </div>
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                            <Label>Jumlah KCP LPU</Label>
                            <div className="w-full md:col-span-3 opacity-80 border-dotted decoration-muted-foreground border-l ps-2 md:ps-3 flex">{selectedData?.jumlah_kpc_lpu}</div>
                        </div>
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                            <Label>Jumlah KCP LPK</Label>
                            <div className="w-full md:col-span-3 opacity-80 border-dotted decoration-muted-foreground border-l ps-2 md:ps-3 flex">{selectedData?.jumlah_kpc_lpk}</div>
                        </div>
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                        ...
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                            ...
                            <div className="w-full md:col-span-3 opacity-80 border-dotted decoration-muted-foreground border-l ps-2 md:ps-3 flex">{selectedData?.alokasi_kcu}</div>
                        </div>
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                            <Label>Rumus Biaya Per KCP</Label>
                            <div className="w-full md:col-span-3 opacity-80 border-dotted decoration-muted-foreground border-l ps-2 md:ps-3 flex">{selectedData?.rumus_biaya_kcp}</div>
                        </div>
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                            <Label>Nominal Biaya Per KCP</Label>
                            <div className="w-full md:col-span-3 opacity-80 border-dotted decoration-muted-foreground border-l ps-2 md:ps-3 flex">{selectedData?.biaya_kcp}</div>
                        </div>
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                            <Label>Rute</Label>
                            <div className="w-full md:col-span-3 opacity-80 border-dotted decoration-muted-foreground border-l ps-2 md:ps-3 flex">{selectedData?.rute}</div>
                        </div>
                        <div className="grid gap-2 md:grid-cols-4 items-center">
                            <Label>Bobot Kiriman</Label>
                            <div className="w-full md:col-span-3 opacity-80 border-dotted decoration-muted-foreground border-l ps-2 md:ps-3 flex">{selectedData?.bobot_kiriman}</div>
                        </div>
                    </div>
                    <div>
            ...
            <div className="absolute h-[45px] lg:h-[55px] bg-gradient-to-r from-cyan-500/25 to-blue-500/50 w-full bottom-0 right-0 space-x-2 flex justify-end items-center p-3 pe-8 z-10">
                <Link
                    href={`./pelaporan-verifikasi?ba_id=${ba_id}&kcu_id=${id_kcu}&triwulan=${getQuarter(parseInt(bulan || ''))}&tahun=${tahun}`}
                    className={cn(
