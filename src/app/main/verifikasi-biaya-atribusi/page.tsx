...
import { VerifikasiAtribusiI, columns } from "./columns";
import { DataTable } from "./data-table";
import { getBiayaAtribusi } from "../../../../services";
...
export default function VerifikasiBiayaAtribusi() {
    const router = useRouter()
    const [queryParams, setQueryParams] = useState<string>('')
  
    const { data, error, isLoading } = useSWR(
      ['VerifikasiBiayaAtribusi',queryParams ? queryParams : '?'],
      (key) => fetcher(key[1])
    );
    const dataSource: VerifikasiAtribusiI[] = data?.data || [];
    const grandTotal: string = data?.grand_total || "";
    
    const fetcher = async (params: string) => {
      try {
        const response = await getBiayaAtribusi(router, params);
    ...
    <div className="px-2 md:px-4 py-1 md:py-2">
        <h3 className="scroll-m-20 text-xl font-semibold tracking-tight mb-3 border-b pb-2">VERIFIKASI BIAYA ATRIBUSI</h3>
        <DataTable 
          columns={columns} 
          data={dataSource} 
          isLoading={isLoading}
