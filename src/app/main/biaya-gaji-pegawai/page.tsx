...
import { getGajiPegawai } from '../../../../services';
import { BiayaGajiPegawaiI } from '../../../../services/types';


export default function BiayaGajiPegawai() {
  const router = useRouter()
  const [queryParams, setQueryParams] = useState<string>('')

  const { data, error, isLoading } = useSWR(
    ['BiayaGajiPegawai', queryParams ? queryParams : '?'],
    (key) => fetcher(key[1])
  );
  const dataSource: BiayaGajiPegawaiI[] = data?.data || [];
  const grandTotal: string = data?.grand_total || "";

  const fetcher = async (params: string) => {
    try {
      const response = await getGajiPegawai(router, params);
