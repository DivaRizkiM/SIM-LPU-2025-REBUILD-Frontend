...
import { getDetailGajiPegawai, getDetailNppNasional, postVerifikasiNppNasional, postVerifikasiPegawai } from "../../../../../services";
...
import { BiayaGajiPegawaiI, IFormGajiPegawai} from "../../../../../services/types";
...
const Detail: NextPage = () => {
    ...
    const params = useParams<{ id_pegawai: string }>()
    const [Datasource, setDataSource] = useState<Array<BiayaGajiPegawaiI>>([])
    const [selectedID, setSelectedID] = useState<string>('')
    const [indexSelected, setIndexSelected] = useState<number>(0)
    const [selectedData, setSelectedData] = useState<BiayaGajiPegawaiI>()
    const [animateTrigger, setAnimateTrigger] = useState<boolean>(false)
    const [dataVerifications, setDataVerifications] = useState<Array<IFormGajiPegawai>>([])
    const [isSubmitting, setIsSubmitting] = useState<boolean>(false)
    const [isLoading, setIsLoading] = useState<boolean>(false)

    const firstInit = async () => {
        const payload = `?id_pegawai=${params.id_pegawai}`
        setIsLoading(true)
        await getDetailGajiPegawai(router, payload)
            .then((res) => {
                const dataResponse: BiayaGajiPegawaiI[] = res.data.data
                if (((res.data as any).isLock)) {
                    router.push(`./`)
                }
                setDataSource(dataResponse)
                const dataEarlier: BiayaGajiPegawaiI = dataResponse[0]
                setSelectedID(dataEarlier.id.toString())
                setSelectedData(dataEarlier)

                const tempDataVerifs: IFormGajiPegawai[] = dataResponse.map((data) => {
                    return {
                        id_pegawai_detail: data.id,
    ...
    const onClickPagination = (data: BiayaGajiPegawaiI, index: number) => {
    ...
    const onSubmitVerifikasi = async () => {
        ...
        const tempData: IFormGajiPegawai[] = dataVerifications.map((data) => {
            return {
                id_pegawai_detail: data.id_pegawai_detail,
        ...
        await postVerifikasiPegawai(router, selectedID, payload)
            .then(() => {
                toast({
    ...
    return (
        <div className="relative px-2 md:px-4 py-1 md:py-2 h-full overflow-hidden">
            ...
            <h3 className="scroll-m-20 text-xl font-semibold tracking-tight mb-3 border-b pb-2">BIAYA LTK - GAJI PEGAWAI</h3>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                            <div className="grid gap-2 md:grid-cols-4 items-center">
                                <Label>Nama</Label>
                                <Input value={selectedData?.nama_pegawai || ''} className="w-full col-span-3 bg-secondary" readOnly />
                            </div>
                            <div className="grid gap-2 md:grid-cols-4 items-center">
                                <Label>Jabatan</Label>
                                <Input value={selectedData?.jabatan || ''} className="w-full col-span-3 bg-secondary" readOnly />
                            </div>
                            <div className="grid gap-2 md:grid-cols-4 items-center">
                                <Label>Nama Bagian</Label>
                                <Input value={selectedData?.nama_bagian || ''} className="w-full col-span-3 bg-secondary" readOnly />
                            </div>
                            <div className="grid gap-2 md:grid-cols-4 items-center">
                                <Label>Kantor</Label>
                                <Input value={selectedData?.kantor || ''} className="w-full col-span-3 bg-secondary" readOnly />
                            </div>
                            <div className="grid gap-2 md:grid-cols-4 items-center">
                                <Label>Periode</Label>
