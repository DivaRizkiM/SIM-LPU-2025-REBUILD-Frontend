...
export function DataTable<TData, TValue>({
  ...
  const id_regional = searchParams.get('regID')
  ...
  return (
    <div className="rounded-md border md:px-3 px-2">
         <div className="flex items-center py-4 justify-end flex-wrap gap-2">
          <Link href={`./verifikasi-kprk?regID=${id_regional}&tahun=${tahun}&triwulan=${triwulan}`} className={cn(
          ...
          onValueChange={(value)=>{
            ...
            <Input
              placeholder="Nama KCP"
              value={(table.getColumn("nama_kpc")?.getFilterValue() as string) ?? ""}
              onChange={(event) =>
                  table.getColumn("nama_kpc")?.setFilterValue(event.target.value)
              }
