...
import { getVerifikasiProduksiByKCU } from "../../../../../services";
import useSWR from "swr";
import { toast } from "@/components/ui/use-toast";

const fetcher = async (url: string, router: any) => {
  const response = await getVerifikasiProduksiByKCU(router, url);
...
export default function VerifikasiKPRK() {
    ...
    const queryParams = `?tahun_anggaran=${tahun}&id_kcu=${id_kcu}&triwulan=${triwulan}`;
    ...
    <div className="px-2 md:px-4 py-1 md:py-2">
        <h3 className="scroll-m-20 text-xl font-semibold tracking-tight mb-3 border-b pb-2">VERIFIKASI PRODUKSI - KPC</h3>
        <DataTable 
          columns={columns} 
          data={dataSource || []} 
          isLoading={isLoading}
