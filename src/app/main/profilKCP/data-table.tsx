...
export function DataTable<TData, TValue>({
  ...
  const onClickPrintQR = async (data: ProfilKcpI) => {
    try {
      setIsDownloadingQR(true);

      const response = await downloadQRKpc(router, data.id);

      // Buat blob dari response dengan type image/png
      const blob = new Blob([response.data], { type: "image/png" });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.style.display = "none";
      a.href = url;
      a.download = `QR_${data.nama}_${data.id}.png`; // Ubah extension jadi .png
      ...
      toast({
        title: `Berhasil mengunduh QR Code ${data.nama}`,
      });
    } catch (error: any) {
      console.error("Error downloading QR:", error);
      toast({
        title: error?.message || "Gagal mengunduh QR Code",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div className="rounded-t-md border" ref={topRef}>
      {isDownloadingQR && (
        <div className="fixed top-0 bottom-0 left-0 right-0 bg-black/5 dark:bg-black/25 z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg flex items-center gap-2">
            <Loader2 className="h-5 w-5 animate-spin" />
            <span>Mengunduh QR Code...</span>
          </div>
        </div>
      )}
      <div className="grid grid-cols-1 md:flex items-center py-4 justify-end flex-wrap gap-2 md:px-3 px-2">
        <div className="md:w-[260px]">
          <Combobox
            value={
              (table.getColumn("id_regional")?.getFilterValue() as string) ?? ""
            }
        ...
        <div className="md:w-[260px]">
          <Combobox
            value={
              (table.getColumn("id_kprk")?.getFilterValue() as string) ?? ""
            }
            options={kprkOptions}
            placeholder={"KC-KCU"}
            onSelect={(value: string) => {
              if (value === "all")
                return table.getColumn("id_kprk")?.setFilterValue("");
              table.getColumn("id_kprk")?.setFilterValue(value);
            }}
            isLoading={isKprkLoading}
            disabled={
              kprkOptions.length <= 0 ||
              isKprkLoading ||
              !table.getColumn("id_regional")?.getFilterValue()
            }
          />
        </div>
        <Input
      ...
      <div className="md:px-3 px-2">
        {table.getRowModel().rows?.length ? (
          table.getRowModel().rows.map((row) => (
            <div
              key={row.id}
              className="relative grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 border dark:bg-gray-900 mb-4 p-4 gap-y-2 shadow-lg rounded dark:hover:bg-gray-900/50 hover:bg-slate-200/50"
            >
                ...
                <div
                  ...
                  className={cn(
                    ...
                    cell.id.includes("actions") &&
                      "col-end-3 sm:col-auto md:col-end-6 border-0"
                  )}
                >
      ...
      <div className="flex lg:flex-row flex-col items-center lg:justify-between py-2 px-2 lg:px-4 rounded-t-md overflow-x-scroll pb-5 mt-2 lg:pb-2">
        ...
        <div className="pagination-summary text-[9px] lg:text-xs items-center w-full lg:w-auto mt-2 md:mt-0 text-center">
          ...
          dari <span className="font-bold">{data.length}</span> item.
        </div>
      </div>
    </div>
  );
