...
import { getProduksiPendapatan } from "../../../../services";
...
export default function DashboardProduksiPendapatan() {
  const router = useRouter()
  const [queryParams, setQueryParams] = useState<string>('')

  const { data, error, isLoading } = useSWR(
    ['getProduksiPendapatan',queryParams ? queryParams : '?'],
    (key) => fetcher(key[1]) 
  );
  const dataSource: ProduksiPendapatanI[] = data?.data || [];
  const grandTotal: string = data?.grand_total || "";

    const fetcher = async (params: string) => {
      try {
        const response = await getProduksiPendapatan(router, params);
    ...
    const firstInit = async (
      ...
      if (tahun && tahun !== "all" && bulan && bulan !== "all") tempParams.tanggal = tahun+parseInt(bulan);
    ...
    <div className="px-2 md:px-4 py-1 md:py-2">
        <h3 className="scroll-m-20 text-xl font-semibold tracking-tight mb-3 border-b pb-2">
          Dashboard Produksi dan Pendapatan (Kalkulasi koefisien)
        </h3>
        <DataTable 
          columns={columns} 
          data={dataSource || []}
