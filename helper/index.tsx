...
export function getQuarter(no_urut_bulan: number) {
  const monthsToQuarters: Record<number, number> = {
    1: 1,
    2: 1,
    3: 1, // Triwulan 1 untuk Januari, Februari, Maret
    4: 2,
    5: 2,
    6: 2, // Triwulan 2 untuk April, Mei, Juni
    7: 3,
    8: 3,
    9: 3, // Triwulan 3 untuk Juli, Agustus, September
    10: 4,
    11: 4,
    12: 4, // Triwulan 4 untuk Oktober, November, Desember
  };
...
// Untuk membersihkan format currency (tampilan ke payload)
...
// Untuk membersihkan format currency HANYA untuk payload (hapus semua format)
export const cleanCurrencyForPayload = (value: string) => {
  // Hapus semua karakter kecuali angka, titik, dan koma
  let cleanedString = value.replace(/[^\d.,]/g, "");

  // Jika ada koma, konversi ke titik desimal
  if (cleanedString.includes(",")) {
    const parts = cleanedString.split(",");
    // Hapus semua titik dari bagian utama (pemisah ribuan)
    const mainPart = parts[0].replace(/\./g, "");
    // Gabungkan dengan titik desimal
    return mainPart + (parts[1] ? "." + parts[1] : "");
  } else {
...
export const isLastPage = (
  total_data_on_a_page: number | undefined,
  pageSize: number
) => {
...
export function separateDate(dateString: number): {
  year: number;
  month: number;
} {
